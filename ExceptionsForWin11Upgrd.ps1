...
try {
    ...
    $exportContent = "Windows 11 Bypass Settings Applied on $(Get-Date)`r`n"
    $exportContent += "================================================`r`n`r`n"
    $exportContent += "SYSTEM\Setup\MoSetup:`r`n"
    $exportContent += (Get-ItemProperty -Path "HKLM:\SYSTEM\Setup\MoSetup" | Out-String)
    $exportContent += "`r`nSYSTEM\Setup\LabConfig:`r`n"
    $exportContent += (Get-ItemProperty -Path "HKLM:\SYSTEM\Setup\LabConfig" | Out-String)
    $exportContent += "`r`nEnvironment Variables Set:`r`n"
    foreach ($var in $envVars.Keys) {
        $exportContent += "$var = $($envVars[$var])`r`n"
    }
    
    $exportContent | Out-File -FilePath $exportPath -Force
